...
### CR-007 — Agent recovery misses initial failures and half-open sockets

Initial `error`/`close` events do not schedule a retry because both
`wasConnected` and `reconnectAttempt` are zero
(`FusionAgentDriver.ets:403-431`). NetworkKit `connect()`, `send()`, and `close()`
rejections are also not routed through a common failure transition.

The driver only answers incoming `ping` at `FusionAgentDriver.ets:478-482`; it
does not send probes, record activity, or close a connection after missed
deadlines. A first-launch race with Agent startup or a half-open bridge can leave
the tab permanently stranded or falsely connected.
