...
assert.match(source, /try \{[\s\S]*const ownSocket = webSocket\.createWebSocket\(\);[\s\S]*this\.socket = ownSocket;[\s\S]*ownSocket\.connect\(url[\s\S]*\.catch\(\(err: Error\)/);
assert.match(source, /ownSocket\.connect\(/);
assert.match(source, /private sendSocketPayload\(payload: string \| ArrayBuffer\): void \{/);
assert.match(source, /try \{[\s\S]*ownSocket\.send\(payload\)[\s\S]*\.catch\(\(err: Error\)[\s\S]*\} catch \(err\) \{/);
...
assert.equal(source.match(/ownSocket\.send\(/g)?.length, 1);
