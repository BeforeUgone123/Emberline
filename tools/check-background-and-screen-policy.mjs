...
assert.match(keepAlive, /backgroundTaskManager\.on\('continuousTaskCancel'/,
  'keep-alive must observe system/user cancellation instead of retaining stale running state');
assert.match(keepAlive, /backgroundTaskManager\.on\('continuousTaskSuspend'/,
  'keep-alive must observe system suspension during long background periods');
assert.match(keepAlive, /backgroundTaskManager\.on\('continuousTaskActive'/,
  'keep-alive must observe system reactivation after suspension');
assert.match(keepAlive, /private ownerContext: common\.UIAbilityContext \| null = null;/,
  'stop must use the UIAbilityContext that actually owns the continuous task');
assert.match(keepAlive, /private transitionGeneration: number = 0;/,
  'async start\/stop completion must be guarded against stale lifecycle transitions');
assert.match(
  keepAlive,
  /if \(this\.activeCount > 0 && this\.lastContext\) \{[\s\S]*?this\.ensureRunning\(this\.lastContext\);/,
  'a completed stop must restart protection when a session became live during the stop'
);
assert.match(index, /keepAlive\.noteSessionActive\(this\.context\);/);
assert.match(index, /keepAlive\.noteSessionInactive\(\);/);
assert.match(index, /keepAlive\.noteAppBackgrounded\(this\.context\);/,
  'background entry must reconcile continuous-task protection before lock\/sleep');
assert.match(index, /keepAlive\.noteAppForegrounded\(this\.context\);/,
  'foreground resume must retry protection after a system cancellation');
