...
界面没有一处强调色。标签上那颗中性小圆点就是全部状态语言:实心是连着,呼吸
是正在连,空心是断开;只有连接失败,才借用当前主题自己的那抹红。
...
- OSC 52 远程复制(远端 tmux/vim 直写系统剪贴板)、OSC 9 与 BEL 桥接为
  HarmonyOS 系统通知(后台标签任务完成会喊你,该标签同时标为未读、标题提亮)。
...
- C++ 原生渲染:脏行级重绘 + 持久离屏 + 滚动位移复用(scroll-damage)+
  ...
  真机 Profiler 回归。
- 窗口全屏且聚焦时系统会走硬件直通合成,这条路径上只上报局部脏区的帧不会
  被送显——曾表现为 tmux 空闲时 codex 的计时器整个卡住,而小窗口或失焦时
  一切正常。现在增量重绘照旧,但每帧按整表面上报 damage。
...
- 已连接 Agent 意外断开后使用指数退避,等待期间敲键可立即重试;首次连接
  失败和半开连接超时仍是发布阻塞项。`taskKeeping` 会跟踪系统取消 / 挂起 /
  恢复事件,并在前后台切换时核对保活;异步启停由代次与实际 owner context
  约束,避免重连期间误停。长锁屏连续性仍须由真机矩阵确认。
- 前台窗口请求常亮,进入后台或销毁窗口时恢复系统休眠策略;这一生命周期
  同样须在目标设备验证。
- 标签是等分的浮动胶囊:按住拿起可在轨道内重排、邻居实时让位,继续垂直
  抬起则进入系统拖拽;双击或右键改名,只剩一个标签时收敛成一行居中标题。
- 多窗口:把标签拖到任意窗口的标签栏即可合并,拖到空白处则在释放位置拆出
  新窗口;右键仍可「拆分为新窗口」或「移动到窗口…」。交接的是活着的会话
  本身,PTY 与 Agent WebSocket 全程不断开。关闭标签同样不打断远端任务。
...
- 图片「中继」视角:在 Agent 会话里 ssh 到另一台机器后,
  粘贴的图片由 wand-agent 从共享目录 `scp` 转发到该机(右键标签 →
  标签设置 → 图片粘贴视角 → 中继,填目标机与远端目录),终端里直接粘出
  目标机上的路径。前提:VM 已配置到目标机的免交互 SSH 凭据;任何失败会
  回退为共享路径并在终端说明原因。
...
- 整个界面的配色由当前主题推导:顶栏直接取终端背景色,与网格无缝相接;文字、
  分隔线、悬浮面板都是前景色的不同透明度。换一套浅色主题,UI 就真的变成浅色,
  不存在「深色壳套浅色终端」。
- 深色与浅色各配一套主题(对应 ghostty 的 `theme = light:…,dark:…`),外观
  可选跟随系统、强制深色或强制浅色。
...
│   ├── src/main/ets/theme/            # 主题派生的界面配色与动效档位
│   ├── src/main/ets/settings/         # 外观 / 连接设置的持久化
...
`dev` 与 `main` 两个通道的 `compatibleSdkVersion` 与 `targetSdkVersion` 现在
都固定为 `6.1.0(23)`(API 23);`compileSdkVersion` 不显式配置,跟随 DevEco
Studio 配套 SDK。DevEco Sync 若提示 SDK version 无效,先确认本机已安装
HarmonyOS 6.1.0(API 23)SDK。该配置不再支持安装到 API 23 以下的设备——正式
版同样受此限制。
...
`service install` 会把服务二进制复制到 `/usr/local/lib/wand-agent/`,与
pnpm/npm 全局包解耦。因此只更新全局包不会改变正在运行的服务。先任选一条
更新包:
...
```sh
wand-agent setup --force &&
wand-agent selftest &&
sudo install -m 755 "$(wand-agent where)" /usr/local/lib/wand-agent/wand-agent &&
sudo systemctl restart wand-agent &&
sudo systemctl status wand-agent --no-pager
```

这条路径不改现有 token、监听地址和 shell 配置。重启会断开所有 Agent
WebSocket,并终止未放在 tmux 中的直连任务;更新前先结束这些任务,或把长期
任务放入 tmux。
...
Emberline 接收 OSC 9/BEL 后会把后台标签标为未读,并发布 HarmonyOS 系统
...
给出对应提示。当前标签在前台时会显示短暂的应用内完成提示;后台标签会标为
未读、标题提亮。两种情况都会进入系统通知链路。系统已启用跨设备通知且配对
...
Profiler。浮动标签(拿起重排、双击改名、单标签收敛)、跨窗口拆分与移动、
自适应顶栏与外观三态、全屏 tmux 刷新已在 dev 真机走过一轮;720/960/1280/
1600 vp 断点矩阵、长 tmux 输出的 Profiler 回归,以及后台 `taskKeeping` 与
熄屏连续性仍未验证。

当前 `dev` 工作树共 40 项。`check-dev-worktree-identity.mjs` 钉住开发通道的
`.debug` 包名、`Emberline Dev` 名称与调试签名,它在正式分支上每条断言都会按
设计失败,因此有意不合入 `main`;其余新增门禁随已评审实现再同步到发布工作树。

干净 clone 仍会因被忽略的 `docs/粘贴板Debug.p7b` 让 `check-terminal-paste.mjs`
失败(该夹具是本机签名材料,从未纳入版本管理)。不要用 `|| echo` 吞掉它,应按
CR-013 修复测试夹具后再把这条命令作为 CI/发布门禁。
...
- **输入**:触控板双指滚动(ToolType 正向路由 / 惯性 / 120Hz 节拍);触屏
  ...
  tmux/vim 鼠标协议桥(触屏合成鼠标,离散滚轮使用短尾惯性);物理键盘快捷键
  与 DECCKM 应用光标键。
...
`upload-relay` 控制消息(把共享目录里的文件 `scp` 转发到远端主机,
